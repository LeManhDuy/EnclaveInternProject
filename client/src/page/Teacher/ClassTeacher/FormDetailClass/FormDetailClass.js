    ...
    StudentService.getStudentByStudentId(props.id)
      .then((response) => {
        if (!!response.studentImage) {
          setAvatarStudent(`${REACT_APP_API_ENDPOINT}${response.studentImage}`);
        }
        if (!!response.ParentImg) {
          setAvatarParents(`${REACT_APP_API_ENDPOINT}${response.ParentImg}`);
        }
        setStudentsInfo({
        ...
        const dataSources = response.ProtectorInformation.map((item, index) => {
          return {
            key: index + 1,
  ...
  const StudentContent = ({ studentsInfo }) => (
    <div className="student-information ">
      ...
      <div className="info-item">
        ...
        <p>{new Date(studentsInfo.studentDateOfBirth).toLocaleDateString()}</p>
      </div>
      <div className="info-item">
        <label>Gender</label>
