...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (admin) {
            ...
            return res.status(200).json({success: true, admin: admin, role: 'admin', type:"Bearer", accessToken})
        }
        if (parent) {
            ...
            if (!validatePassword)
                return res.status(400).json({success: false, message: 'Incorrect email or password'})
            return res.status(200).json({success: true, parent: parent, type:"Bearer", role: 'parent', accessToken})
        }
        if (teacher) {
            ...
            if (!validatePassword)
                return res.status(400).json({success: false, message: 'Incorrect email or password'})
            return res.status(200).json({success: true, teacher: teacher, type:"Bearer", role: 'teacher', accessToken})
        }
